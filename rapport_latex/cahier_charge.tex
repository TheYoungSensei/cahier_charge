...
\begin{itemize}
	...
	\item Rouvert
\end{itemize}
Le passage d'un état à l'autre symbolise un changement dans la façon de traiter ce bug et son avancée dans le processus de résolution. Il faudra donc notifier les utilisateurs de l'évolution du projet via ces différents états.\footnote{Si un bug clos veut être rouvert, on ne changera pas l'état de ce dernier, mais on considèrera qu'il s'agisse d'un nouveau bug.}
...
\subsubsection{Introduire une fiche signalétique, une fiche descriptive ou un RFE}
\subsubsection{Rechercher un bug ou une RFE spécifique}
\subsection{Le membre d'équipe de développement}
...
\subsubsection{Modifier une fiche signalétique ou descriptive}
\subsubsection{Passer un bug ou une RFE en état doublon}
\subsubsection{Passer un bug ou une RFE en état non-reproductible}
\subsection{Le responsable d'équipe de développement}
\subsubsection{Confirmer la pertinence du bug ou d'une RFE}
\subsubsection{Clôre un bug ou une RFE}
...
\end{figure}
\section{Spécifications de réalisations}
